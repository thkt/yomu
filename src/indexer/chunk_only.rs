...
use crate::storage::{self, Db, SourceKind, StorageError};

pub(super) use references::build_references;

pub(super) struct PendingFile {
    pub(super) rel_path: String,
    pub(super) raw_chunks: Vec<chunker::RawChunk>,
    imports_text: String,
    ...
    pub(super) injection_flags: Vec<String>,
}

impl PendingFile {
    pub(super) fn to_new_chunks(&self) -> Vec<storage::NewChunk<'_>> {
        self.raw_chunks
            .iter()
...
pub(super) const MAX_FILE_SIZE: u64 = 1_000_000;
...
pub(super) fn to_rel_path(root: &Path, file_path: &Path) -> String {
...
pub(super) fn read_source(file_path: &Path) -> Result<String, FileAction> {
...
pub(super) fn file_hash(content: &str) -> String {
