...
async fn search_shows_coverage_on_no_results() {
    ...
    storage::replace_file_chunks_only(
        ...
        &[storage::NewChunk {
            ...
            name: Some("A"), content: "function A() {}",
...
async fn search_degraded_empty_results_shows_note() {
    ...
    assert!(text.contains("No results found"), "expected no results: {text}");
    assert!(text.contains("embedding"), "expected embedding note in empty results: {text}");
}
...
async fn search_degraded_with_results_shows_note() {
    ...
    indexer::run_index(
        Arc::clone(&y.conn), y.root.as_path(),
    ...
    assert!(result.contains("embedding API unavailable"), "should show degraded note");
}
...
fn format_results_grouped_renders_file_header_and_context() {
    ...
    let ctx = EnrichmentContext { imports: imports_map, siblings: siblings_map };
    let text = format_results_grouped(&results, &ctx);
    assert!(text.contains("## src/Button.tsx"), "missing file header: {text}");
    assert!(text.contains("Imports: import React from 'react'"), "missing imports: {text}");
    assert!(text.contains("Siblings: ButtonProps [type_def]"), "missing siblings: {text}");
...
fn format_results_grouped_groups_same_file_chunks() {
    let results = vec![
        storage::SearchResult {
            ...
            chunk_id: None, distance: 0.1,
            match_source: storage::MatchSource::Semantic, score: 1.0 / (1.0 + 0.1),
        },
        storage::SearchResult {
            chunk: storage::Chunk {
                file_path: "src/Form.tsx".to_string(),
            ...
            chunk_id: None, distance: 0.2,
            match_source: storage::MatchSource::Semantic, score: 1.0 / (1.0 + 0.2),
        },
    ];
    let ctx = EnrichmentContext { imports: HashMap::new(), siblings: HashMap::new() };
    let text = format_results_grouped(&results, &ctx);
    assert_eq!(text.matches("## src/Form.tsx").count(), 1, "expected one file header: {text}");
...
fn format_results_grouped_deduplicates_siblings() {
    let results = vec![storage::SearchResult {
        chunk: storage::Chunk {
            file_path: "src/A.tsx".to_string(), chunk_type: storage::ChunkType::Component,
            name: Some("A".to_string()), content: "function A() {}".to_string(),
            start_line: 5, end_line: 7,
        },
        chunk_id: None, distance: 0.1, match_source: storage::MatchSource::Semantic,
        score: 1.0 / (1.0 + 0.1),
    }];
    let siblings_map = HashMap::from([(
        "src/A.tsx".to_string(),
        vec![
            storage::SiblingInfo { name: Some("A".to_string()), chunk_type: storage::ChunkType::Component, start_line: 5, end_line: 7 },
            storage::SiblingInfo { name: Some("AProps".to_string()), chunk_type: storage::ChunkType::TypeDef, start_line: 1, end_line: 3 },
        ],
    )]);
    let ctx = EnrichmentContext { imports: HashMap::new(), siblings: siblings_map };
    let text = format_results_grouped(&results, &ctx);
    assert!(text.contains("AProps [type_def]"), "sibling should be included: {text}");
    let siblings_line = text.lines().find(|l| l.starts_with("Siblings:")).unwrap();
    assert!(!siblings_line.contains("A [component]"), "search result should be excluded from siblings: {siblings_line}");
}
...
fn format_results_grouped_omits_empty_imports() {
    let results = vec![storage::SearchResult {
        chunk: storage::Chunk {
            file_path: "src/A.tsx".to_string(), chunk_type: storage::ChunkType::Component,
            name: Some("A".to_string()), content: "code".to_string(),
            start_line: 1, end_line: 3,
        },
        chunk_id: None, distance: 0.1, match_source: storage::MatchSource::Semantic,
        score: 1.0 / (1.0 + 0.1),
    ...
    let ctx = EnrichmentContext { imports: imports_map, siblings: HashMap::new() };
    let text = format_results_grouped(&results, &ctx);
...
fn format_results_grouped_omits_empty_siblings() {
    ...
    let ctx = EnrichmentContext { imports: HashMap::new(), siblings: siblings_map };
    let text = format_results_grouped(&results, &ctx);
    assert!(!text.contains("Siblings:"), "empty siblings should be omitted: {text}");
}
...
fn format_results_grouped_sorts_files_by_best_similarity() {
    let results = vec![
        storage::SearchResult {
            chunk: storage::Chunk {
                file_path: "src/B.tsx".to_string(), chunk_type: storage::ChunkType::Component,
                name: Some("B".to_string()), content: "code B".to_string(),
                start_line: 1, end_line: 3,
            },
            chunk_id: None, distance: 0.5, match_source: storage::MatchSource::Semantic,
            score: 1.0 / (1.0 + 0.5),
        },
        storage::SearchResult {
            chunk: storage::Chunk {
                file_path: "src/A.tsx".to_string(), chunk_type: storage::ChunkType::Component,
                name: Some("A".to_string()), content: "code A".to_string(),
                start_line: 1, end_line: 3,
            },
            chunk_id: None, distance: 0.1, match_source: storage::MatchSource::Semantic,
            score: 1.0 / (1.0 + 0.1),
    ...
    assert!(a_pos < b_pos, "A (better similarity) should come before B: {text}");
}
...
fn format_results_grouped_shows_score_for_all() {
    let results = vec![
        storage::SearchResult {
            ...
            chunk_id: None, distance: 0.5, match_source: storage::MatchSource::Semantic, score: 0.72,
        },
        storage::SearchResult {
            chunk: storage::Chunk {
                file_path: "src/B.tsx".to_string(), chunk_type: storage::ChunkType::Hook,
                name: Some("useAuth".to_string()), content: "function useAuth() {}".to_string(),
                start_line: 1, end_line: 3,
            },
            chunk_id: None, distance: f32::INFINITY, match_source: storage::MatchSource::NameMatch, score: 0.55,
        },
    ];
    let ctx = EnrichmentContext { imports: HashMap::new(), siblings: HashMap::new() };
    let text = format_results_grouped(&results, &ctx);
...
fn format_results_grouped_sorts_by_score() {
    let results = vec![
        storage::SearchResult {
            ...
            chunk_id: None, distance: 0.5, match_source: storage::MatchSource::Semantic, score: 0.60,
        },
        storage::SearchResult {
            chunk: storage::Chunk {
                file_path: "src/A.tsx".to_string(), chunk_type: storage::ChunkType::Component,
            ...
            chunk_id: None, distance: 0.1, match_source: storage::MatchSource::Semantic, score: 0.95,
        },
    ];
    let ctx = EnrichmentContext { imports: HashMap::new(), siblings: HashMap::new() };
    let text = format_results_grouped(&results, &ctx);
...
async fn status_returns_empty_stats() {
    ...
    assert!(text.contains("References: 0"), "expected 0 references, got: {text}");
...
async fn impact_lists_dependents() {
    ...
    assert!(text.contains("Depth"), "expected depth heading in output: {text}");
}
...
async fn impact_filters_by_symbol() {
    ...
    let text = y.impact("src/hooks/useAuth.ts:useAuth", None, 3).await.unwrap();
...
async fn impact_errors_on_empty_index() {
    ...
    assert!(err.to_string().contains("index is empty"), "expected empty index error, got: {err}");
}
...
async fn impact_distinguishes_missing_file() {
    ...
    assert!(text.contains("not found in index"), "expected file-not-found message: {text}");
}
...
async fn impact_rejects_path_traversal() {
    ...
    assert!(err.to_string().contains(".."), "expected path traversal error, got: {err}");
}
...
async fn impact_symbol_flag_overrides_colon_syntax() {
    ...
    let text = y.impact("src/hooks/useAuth.ts:useAuth", Some("AuthProvider"), 3).await.unwrap();
    assert!(text.contains("src/B.tsx"), "expected B.tsx for AuthProvider: {text}");
}
...
fn determine_index_state_variants() {
    let empty = storage::IndexStatus {
        total_files: 0, total_chunks: 0, embedded_chunks: 0, last_indexed_at: None,
    };
    assert!(matches!(determine_index_state(&empty), IndexState::Empty));

    let chunked = storage::IndexStatus {
        total_files: 5, total_chunks: 20, embedded_chunks: 0,
        last_indexed_at: Some("2026-01-01".into()),
    };
    assert!(matches!(determine_index_state(&chunked), IndexState::ChunkedOnly));

    let partial = storage::IndexStatus {
        total_files: 5, total_chunks: 20, embedded_chunks: 10,
        last_indexed_at: Some("2026-01-01".into()),
    };
    assert!(matches!(determine_index_state(&partial), IndexState::PartiallyEmbedded));

    let full = storage::IndexStatus {
        total_files: 5, total_chunks: 20, embedded_chunks: 20,
        last_indexed_at: Some("2026-01-01".into()),
    };
    assert!(matches!(determine_index_state(&full), IndexState::FullyEmbedded));
}
...
async fn ensure_indexed_fully_embedded_with_failing_embedder() {
    ...
    assert!(result.contains("Card"), "should find Card with existing embeddings: {result}");
}
