...
インデックスの同期遅延、陳腐化したエンベディング、コールドスタートなどRAGの定番の問題には、インデックス構築を明示的にしクエリを読み取り専用に保つことで対処しています。

| 対策                   | 内容                                                                                |
| ---------------------- | ----------------------------------------------------------------------------------- |
| 読み取り専用クエリ     | `search` / `impact` はインデックスを変更しないため、エージェントが何度でも呼べる    |
| 高速な差分インデックス | `yomu index` は変更ファイルのみ（mtime 基準）を再チャンク、3,535ファイルで約2.5秒   |
| APIキー不要            | ローカルエンベディングモデル、モデル利用不可時はFTS5全文検索にフォールバック        |
| ワンパスエンベディング | `index` がチャンク化とエンベディングを1パスで実行（ADR-0002）。クエリ時のコストなし |
...
インデックスは明示的に行います。検索前に `yomu index` を実行してください。クエリ（`search` / `impact`）は読み取り専用で、自動インデックスはしません。
...
| コマンド  | 内容                                                                                                                                  |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `index`   | チャンクインデックスを更新する（チャンク + エンベディングを1パス）。API呼び出しなし、3,535ファイルで約2.5秒。ファイル変更後に実行する |
| `rebuild` | ゼロからすべて再パースする                                                                                                            |
| `status`  | ファイル数、チャンク数、エンベディングカバレッジ、リファレンス数を表示する                                                            |

クエリ（`search` / `impact`）は読み取り専用で、再インデックスはしません。いつ再インデックスするかは consumer の責務です。例えば Claude Code の `Stop` フックや `PostToolUse(Edit|Write)` フックから `yomu index` を実行すると楽です。`index` は変更ファイルのみ（mtime 基準）を再チャンクするため、再実行は安価で冪等です。
...
| エンベディング   | チャンクは `yomu index` 実行時にローカルモデル（Ruri v3、310Mパラメータ）でチャンク化と同じパスでエンベディングされる（ADR-0002）。API呼び出しなし、クエリ時のコストなし                                                                 |
...
ADR-0066 Group 2 (sysexits.h) に準拠。エージェントやスクリプトは数値で retry policy を分岐できる。`--json` 時はエラーが 1 行 envelope で stderr に出力される: `{"error":{"code":"<CODE>","message":"<text>"}}`

| Code | `error.code`   | 発生条件                                                                            |
| ---- | -------------- | ----------------------------------------------------------------------------------- |
| 0    | (none)         | 成功                                                                                |
| 64   | `USAGE_ERROR`  | CLI 入力不正 — clap parse 失敗、空 query / task、数値引数の範囲外                   |
| 70   | `INTERNAL`     | 不変条件違反、search / query 失敗                                                   |
| 73   | `CANT_CREAT`   | index DB 作成 / migrate 失敗、indexing パイプライン失敗                             |
| 74   | `IO_ERROR`     | ファイルシステム IO 失敗                                                            |
| 75   | `TEMP_FAILURE` | embedder 利用不可（モデル未配置 / probe 失敗）— `yomu model download` 後に retry 可 |
| 104  | `UNKNOWN`      | 分類不能の失敗用予約（現状未使用 — 経路が漏れた場合に surface する）                |

> **破壊的変更 (0.16+)**: 旧バージョンは失敗時 `1` / `2` / `4` を返していた。これらの数値で分岐するスクリプトは sysexits 値への移行が必要。テキストモードの stderr は従来通り `error: <message>` 形式。

> **注意**: clap の引数 parse 失敗 (未知の flag, 範囲外の数値, subcommand 欠落など) は `--json` 指定でも exit `64` で text-mode の `error: <message>` を stderr に出す。`--json` は parse 後に解決されるため、parse 時点の失敗には適用されない。
...
| コールドスタート         | 初回の `yomu index` がモデルをロードして全チャンクを埋め込む（数秒）                      |
...
```sh
cargo nextest run --features test-support                 # 全テスト（test-support が埋め込みモデルをスタブ化し index をモデルなしで実行可能にする。導入: cargo install cargo-nextest --locked）
