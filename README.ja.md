...
| APIキー不要                    | ローカルエンベディングモデル、モデル未ダウンロード時はFTS5全文検索にフォールバック              |
...
セマンティック検索にはローカルエンベディングモデル（[Ruri v3](https://huggingface.co/cl-nagoya/ruri-v3-310m)、約1.2 GB）を使用します。初回の `search` 呼び出し時に自動ダウンロードされます。APIキーは不要です。

モデルが利用できない場合（オフライン、ディスク容量不足など）、`search` はテキスト検索モード（FTS5）にフォールバックします。その他のコマンド（`index`、`rebuild`、`impact`、`status`）はモデル不要です。
...
| 制限                     | 内容                                                                                                                          |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------- |
| 初回モデルダウンロード   | 初回の `search` 呼び出し時に約1.2 GBのダウンロードが発生する（ダウンロード後はキャッシュされる）                              |
